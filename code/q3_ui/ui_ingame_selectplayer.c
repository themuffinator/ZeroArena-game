...
typedef struct {
	...
	char			playerString[12];	// [MAX_SPLITVIEW] [12] ;
	void 			(*playerfunc)(int);
} setupplayersmenu_t;
...
void InSelectPlayer_MenuInit( const char* banner, qboolean disableMissingPlayers ) {
	...
	Com_sprintf( s_setupplayers.playerString, sizeof( s_setupplayers.playerString ), "Player" );
	s_setupplayers.player.generic.type = MTYPE_PTEXT;
	s_setupplayers.player.generic.flags = QMF_CENTER_JUSTIFY | QMF_PULSEIFFOCUS;
	s_setupplayers.player.generic.x = 320;
	s_setupplayers.player.generic.y = y;
	s_setupplayers.player.generic.id = ID_CUSTOMIZEPLAYER;	// +i;
	s_setupplayers.player.generic.callback = InSelectPlayer_Event;
	s_setupplayers.player.string = s_setupplayers.playerString;
	if ( !disableMissingPlayers ) {
		// Have players in game be red and not ingame be white.
		if ( cg.localPlayers[0].playerNum == -1 ) {
			s_setupplayers.player.color = color_white;
		} else {
			s_setupplayers.player.color = color_red;
		}
	} else {
		s_setupplayers.player.color = text_big_color;
	}
	s_setupplayers.player.style = UI_CENTER | UI_SMALLFONT;

	if ( disableMissingPlayers && cg.localPlayers[0].playerNum == -1 ) {
		s_setupplayers.player.generic.flags |= QMF_GRAYED;
	}
	...
	Menu_AddItem( &s_setupplayers.menu, &s_setupplayers.player );
