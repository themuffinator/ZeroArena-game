...
void AddScore( gentity_t* ent, vec3_t origin, int score ) {
	...
	if ( GTF( GTF_TEAMS ) && GTL( GTL_FRAGS ) ) {
...
Drop CTF flag and Harvester skulls
===========
*/
void TossPlayerGametypeItems( gentity_t* ent ) {
	...
	if ( GTL( GTL_CAPTURES ) ) {
		...
		if ( item ) {
			...
			angle += 45;
			// decide how many seconds it has left
			drop->count = (ent->player->ps.powerups[i] - level.time) / 1000;
			if ( drop->count < 1 ) {
				drop->count = 1;
			}
...
Spawn skull at neutral obelisk
=================
*/
...
void TossPlayerSkulls( gentity_t* self ) {
	...
	for ( i = FIRST_TEAM; i <= level.teams_max; i++ ) {
		item = BG_FindItem( va( "%s Skull", g_teamNames[self->player->sess.sessionTeam] ) );
	}
...
==================
*/
static qboolean CheckGoalNear( gentity_t* self, char* classname ) {
	gentity_t* ent;
	vec3_t		dir;
	...
	// if we found the destination goal and it's not picked up
...
void CheckAlmostCapture( gentity_t* self, gentity_t* attacker ) {
	...
	if ( BG_CarryingCapturableFlag( &self->player->ps, g_gameType.integer ) >= 0 ) {
		...
		if ( GTF( GTF_CTF ) ) {
			...
			int i;
			// GT_1FLAG: we have to run through each enemy team flag entity
			for ( i = FIRST_TEAM; i <= level.teams_max; i++ ) {
...
void player_die( gentity_t* self, gentity_t* inflictor, gentity_t* attacker, vec3_t dir, int damage, int meansOfDeath ) {
	...
	// check for a player that almost brought in skulls
	...
	if ( attacker && attacker->player ) {
		...
		if ( meansOfDeath == MOD_SUICIDE_TEAM_CHANGE ) {
			AddScore( self, self->r.currentOrigin, 0 );
		} else {
	...
	if ( meansOfDeath == MOD_SUICIDE || meansOfDeath == MOD_SUICIDE_TEAM_CHANGE ) {
		const int flagCheck = BG_CarryingCapturableFlag( &self->player->ps, g_gameType.integer );

		if ( flagCheck >= 0 ) {
			Team_ReturnFlag( flagCheck );
			self->player->ps.powerups[PW_FLAGS_INDEX + flagCheck] = 0;
		}
	}
...
static int CheckArmor( gplayer_t* player, int damage, int dFlags ) {
	...
	else if ( save < 1 ) return 0;

	player->ps.stats[STAT_ARMOR] -= save;
#if 0
	if ( g_debugArmor.integer ) {
		G_Printf( "%i: Client:%i armorType:%i armorProtection:%f armorDamage:%i * %f = %i\n\n", level.time, ent->s.number,
			player->ps.stats[STAT_ARMOR_TYPE], armorProtection, damage, armorProtection, save );
	}
#endif
	return save;
}
...
//-muff
	...
	if ( g_gameType.integer == GT_OVERLOAD && CheckObeliskAttack( targ, attacker ) ) {
		return;
	}
